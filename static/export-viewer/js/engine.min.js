(function(){
  ...
  function normalizeDeg(d){ if(!Number.isFinite(d)) return 0; d = d % 360; if(d < 0) d += 360; return d; }
  ...
  var dampening=0.95;  // editor.dampening
    ...
    function getPanoTexture(path){
      ...
      return new Promise(function(resolve, reject){
        ...
        panoLoader.load(path, function(tex){ 
          ...
          try {
            // Match editor texture sampling to reduce artifacts near poles
            ...
            // Prefer maximum available anisotropy for sharper look at grazing angles
            var maxAniso = (renderer && renderer.capabilities && renderer.capabilities.getMaxAnisotropy)
              ? renderer.capabilities.getMaxAnisotropy() : (renderer.capabilities && renderer.capabilities.anisotropy) || 0;
            if (maxAniso && typeof tex.anisotropy === 'number') tex.anisotropy = maxAniso;
          } catch(e) { /* ignore */ }
    ...
    function setViewFromScene(sc){
      var nd = normalizeDeg(sc.north_dir || 0);
      if (typeof sc.initial_view_x === 'number') {
        lon = normalizeDeg(sc.initial_view_x - nd); // compensate for world rotation
      } else { lon = 0; }
      if (typeof sc.initial_view_y === 'number') { lat = sc.initial_view_y; } else { lat = 0; }
      if (typeof sc.initial_fov === 'number') camera.fov = sc.initial_fov;
      camera.updateProjectionMatrix();
      if (window.__vtDebug) {
        console.log('[VT] setViewFromScene', { raw_initial_view_x: sc.initial_view_x, north_dir: nd, adjusted_lon: lon, lat: lat, fov: camera.fov });
      }
    }
    ...
    function buildPanorama(tex){
      ...
      // Increase segment counts to match editor (reduces polar distortion/"pinch").
      var geom = new THREE.SphereGeometry(500.2, 120, 80);
      geom.scale(-1, 1, 1);
  ...
  function placeHotspots(sc){
      ...
      sc.connections.forEach(function(conn, idx){
        ...
        if (parsed) { parsed.lon = normalizeDeg(parsed.lon); }
        var distance = 490;
        ...
        world.add(spr);
        hotspots.push(spr);
        console.log('[VT] Hotspot positioned:', { idx: idx, type: conn.connection_type, name: conn.name, distance: distance, pos: {x:v.x,y:v.y,z:v.z}, rawLonLat: conn.position, adjusted_lon: parsed ? parsed.lon : undefined });
        if (idx < 10) dbg('hotspot', { idx: idx, type: conn.connection_type, name: conn.name, target: conn.target_scene_id, icon_index: conn.icon_index, rawPos: conn.position, adjusted: parsed, posLen: v.length(), pos: {x:v.x,y:v.y,z:v.z} });
      });
    }
